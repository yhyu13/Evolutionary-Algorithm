...
CONFIG = [
    ...
    dict(game="opensim",
         n_feature=41, n_action=18, continuous_a=[True, 1.], ep_max_step=1000, eval_threshold=2)
][3]    # choose your game
...
class ADAM(object):                      # optimizer with momentum
    ...
    def get_gradients(self, gradients):
        ...
        v_t = (b2 * T.sqr(gradients)) + ((1. - b2) * self.v)
...
def get_action(params, x, continuous_a):
    ...
    if not continuous_a[0]:
        ...
        return continuous_a[1] * sigmoid(x)[0]                # for continuous action
